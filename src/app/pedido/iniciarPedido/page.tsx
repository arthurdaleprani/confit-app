...
export default function Home() {
  ...
  // Calcula a data mínima (hoje + 2 dias)
  const minDate = new Date(Date.now() + 2 * 24 * 60 * 60 * 1000);
  const minDateStr = minDate.toISOString().split("T")[0];

  useEffect(() => {
    // define a data inicial no localStorage
    localStorage.setItem("dataEvento", minDateStr);
  }, [minDateStr]);
    ...
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <main className="flex-1 flex flex-col items-center justify-center text-center p-4">
        ...
        <div className="mb-10 w-full max-w-[200px]">
          <label htmlFor="event-date" className="text-gray-600 mb-3 block text-sm font-light">
            Data do seu evento:
          </label>
          ...
          <input
            ...
            onChange={(e) => localStorage.setItem("dataEvento", e.target.value)}
          />
        </div>
